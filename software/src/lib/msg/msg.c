	...
	MsgServer	s;		/* Server handle	*/
{
    ...
    if ( (s->clnts[i].inp  = Accept(s->clnts[0].inp, "r+", &(s->clnts[i].address))) == NULL ) {
    	msg_log(s, "New Client: Accept rejected");
	return 0;
    }
    ...
    msg_log(s, "New Client: %s #%d on fd %d"
		, s->clnts[i].address
    		, i
	...
	Client		c;		/* Client handle	*/
{
    ...
    msg_log(s, "Kil Client: %s", c->address);
...
/*fprintf(stderr, "Handeling input with s = 0x%x\n", s); */
	...
	char		*argv[];
{
	...
	if ( s->clnts[0].type == MsgServerSide ) {
	    ...
	    msg_log(s, "unexpected data block: %d", leng);
	...
	if ( !blk ) {
	    msg_log(s, "expected data block: %d", a->leng);
	}
	leng = atoi(argv[1]);

	if ( leng > a->leng ) {
	    msg_log(s, "data block length mismatch: expected %d got %d"
		   	, a->leng, leng);
	}
	...
	char		*argv[];
{
    ...
    switch ( value->type ) {
     ...
     case MsgBlkType: msg_rpy(c, msgid, "blk", "%d"   , value->leng);
		      Write(c->out, data, value->leng, 1);
		      Flush(c->out);
    }
...
void  isrv_tok(s, c, msgid, data, argc, argv)
	MsgServer	 s;
	Client		 c;
	void		*data;
	int		 msgid;
	...
	MsgServer	s;
{
    ...
    if ( s->up ) {
	msg_log(s, "reopen: already up on %s",s->name);
	return 0;
    }
    msg_log(s, "DEBUG reopen: on %s",s->name);
...
#define MSGGRP 4
	...
	if ( s->nvalues ) { 
	    ...
	    msg_log(s, "reopen: opened server %s", s->sok);
	    return 0.0;
	}
    }
...
ErrorInSubs:
	...
	if ( !(s->tid = msg_addtimer(s, interval, msg_reopen, NULL)) ) {
    	    msg_log(s, "reopen: addtimer fails!!!!\n");
        }
    }

    msg_log(s, "reopen: failed, try again in %f", interval);
    return interval;
}
	...
	va_list	   args;
{
    if ( msg_debug  ) {
#ifdef _REENTRANT
        fprintf(stderr, "Thread %d: ", pthread_self()); 
#endif
...
#ifdef __STDC__
/* log a message */
int msg_log(MsgServer s, char *format, ...)
{
    va_list         args;
    va_start(args, format);
#else
int msg_log(va_alist) va_dcl
{
    va_list         args;
