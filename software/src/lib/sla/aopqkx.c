#include "slalib.h"
#include "slamac.h"
void slaAopqkx(rap, dap, aoprms, aob, zob, hob, dob, rob, flags)
     double rap;
     double dap;
     double *aoprms;
     double *aob;
...
**  - - - - - - - - -
**
**  Quick apparent to observed place.  With switches to control which 
**  corrections to execute.
**
**  Given:
...
*/
   ...
   double sphi, cphi, st, v[3], xhd, yhd, zhd,
          diurab, f, xhdt, yhdt, zhdt, xaet, yaet, zaet, azobs,
          xaeo, yaeo, zaeo, zdt, zdobs, dref, dzd,
          refold, ce, hmobs, dcobs, raobs;
...
/* Large zenith distance? */
 ...
 if ( flags & 0x0002 )
   ...
   /* Fast algorithm using two constant model */
      slaRefz ( zdt, aoprms[10], aoprms[11], &zdobs );
   ...
   /* Maximum of 10 iterations */
      ...
      /* Otherwise, if not converged, use Newton-Raphson */
         else if ( fabs ( dzd ) > 1e-16) {
            dzd = ( zdobs + dref - zdt ) /
                  ( 1.0 + ( refold - dref ) / dzd );
            zdobs -= dzd;
            refold = dref;
