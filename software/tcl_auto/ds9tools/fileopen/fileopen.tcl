...
#
# RCS: @(#) $Id: fileopen.tcl,v 1.1.1.1 2001/04/05 15:24:17 mo Exp $
#
...
proc ::tk::dialog::file::tkFDialog {type args} {
    ...
    set pattern 0
        ...
        set blueplate $data(-blueplate)
        set loadtype  $data(-loadtype)
        set pattern  $data(-pattern)
        set dirslist $data(-dirslist)
    }
	...
	::tk::dialog::file::Create $w TkFDialog
          ...
          ||  $blueplate != $data(-blueplate)                   \
          ||  $loadtype  != $data(-loadtype)                    \
          ||  $dirslist  != $data(-dirslist)                    \
    ...
    if { $blueplate == 0 } { ::tk::SetFocusGrab $w $data(ent) }
    ...
    if { $blueplate == 0 } { 	::tk::RestoreFocusGrab $w $data(ent) withdraw
    } else { 			wm withdraw $w }
...
proc ::tk::dialog::file::Create {w class} {
    ...
    # set the colormap, if private
    global ds9
    if [info exists ds9(main)] {
	toplevel $w -class $class -colormap $ds9(main)
    } else {
...
static char updir_bits[] = {
   ...
   if $data(-blueplate) {
    set data(files) [tkIconList $w.files \
        -browsecmd "::tk::dialog::file::ListBrowse $w" \
        -command   $iconListCommand]
   }
 
   # f2a: the frame with the new controls in it.
   #
   global $w.icons
   if $data(-blueplate) {
     global $w.files
   ...
   if { $data(-loadtype) != {} } {
        ...
        set f2a [frame $w.f2a -bd 0]
 
        label $f2a.lab -text "Open As:"
        set data(loadType) [eval tk_optionMenu $f2a.menu FileLoadType $data(-loadtype)]
        foreach item $data(-loadtype) {
            $data(loadType) entryconfigure $item -indicatoron 0
        }
        pack $f2a.lab $f2a.menu -side left -fill x
   }
   if $data(-blueplate) {
    if { ![winfo exists $w.f2a] } {
         set f2a [frame $w.f2a -bd 0]
    }
 
    label $f2a.show -text "  Show:"
    set data(fDots)     [checkbutton $f2a.fdots     -text "Dot Files" -width 12 \
        -pady 3 -variable $w.files(-dots)]
    set data(dDots)     [checkbutton $f2a.ddots     -text "Dot Dirs"  -width 12 \
        -pady 3 -variable $w.icons(-dots)]
    set data(dScan)     [button $f2a.dscan     -text "Rescan"     -under 0 -width 6 \
        -pady 3 ]
   }
    ...
    if { $data(-pattern) } {
        global FileNameType
        set data(nameType) [tk_optionMenu $f2.lab FileNameType {File name:} {File pattern:}]
        foreach item {{File name:} {File pattern:}} {
            $data(nameType) entryconfigure $item -indicatoron 0
        }
    } else {
        global FileNameType 
        set FileNameType "File name:"
    ...
    set $w.files(font) [$data(ent) cget -font]
    ...
    #
   if $data(-blueplate) {
    pack $f2a.show $data(fDots) $data(dDots) -side left -expand yes -fill x
    pack $data(dScan) -side right -padx 9 -anchor e
   }
   ...
   if $data(-blueplate) {
    pack $f2a -side bottom -fill x
   }
    pack $data(icons) -expand yes -fill both -padx 4 -pady 1
   if $data(-blueplate) {
    pack $data(files) -expand yes -fill both -padx 4 -pady 1
   }
	...
	bind $w <Alt-s> [list ::tk::dialog::file::InvokeBtn $w Save]
       ...
       if $data(-blueplate) {
	$data(dDots)     config -command "::tk::dialog::file::UpdateWhenIdle $w"
	$data(fDots)     config -command "::tk::dialog::file::UpdateWhenIdle $w"
	$data(dScan)     config -command "::tk::dialog::file::UpdateWhenIdle $w"
       }
...
rSASvJTGhnhcV3EJlo3kh53ltF5nAhQAOw==}]
   ...
   if $data(-blueplate) {
    tkIconList_DeleteAll $data(files)
   }


    # Make the dir list
    #
    set boxname $data(icons)
    set dots .*
   ...
   if $data(-blueplate) {
    ...
    foreach f [lsort -dictionary [glob -nocomplain $dots *]] {
	if {[string equal $f .]} {
	    continue
	}
	if {[string equal $f ..]} {
	...
	foreach f $files {
		...
		if {![info exists hasDoneFile($f)]} {
		    tkIconList_Add $boxname $file $f
		    set hasDoneFile($f) 1
		}
	    }
	}
       if $data(-blueplate) {
	tkIconList_Arrange $data(files)
       }
    }
    ...
    if { ![string compare $home $appPWD] } {
        set home ""
    }

    $data(dirMenu) delete 0 end
    set var [format %s(selectPath) ::tk::dialog::file::$dataName]
    set pathlist "$home $appPWD $data(-dirslist)"
    foreach path $pathlist {
        if { [string compare $path $data(selectPath)] } {
            $data(dirMenu) add command -label $path -command [list set $var $path]
        }
    }
    if { [string compare $data(-dirslist) {}] } {
...
#	      = ERROR	: Invalid entry
#             = NOMATCH : Can't glob in FileNameType mode == File pattern
#
...
proc ::tk::dialog::file::ResolveFile {context text defaultext} {
    ...
    global FileNameType
    if { ![string compare $FileNameType "File pattern:"] } {
        if [catch { set text [glob $context/$text] }] {
            return [list NOMATCH $text ""]
        } else {
            return [list OK $context $text]
        }
    } 
...
# we want to strip any filtering/ext/blocking instructions from the file name
...
proc UpdateFitsMenu {} {
    ...
    UpdateCoordMenu2 .menuBar.frame.match 0 0
    # not implemented yet
    .menuBar.frame.match entryconfig "Detector" -state disabled
    .menuBar.frame.match entryconfig "Amplifier" -state disabled
    .menuBar.frame.match entryconfig "Physical" -state disabled

    UpdateCoordMenu2 .menuBar.frame.crosshair 0 0
    AdjustCoordSystem crosshair(lock)

    if {$current(frame) != ""} {
	.menuBar.file entryconfig "Open..." -state normal
#	.menuBar.file entryconfig "Open Array..." -state normal
	.menuBar.file entryconfig "Open URL..." -state normal
#	.menuBar.file entryconfig "Open Mosaic Image File..." -state normal
#	.menuBar.file entryconfig "Open Mosaic Image Multiple Files..." \
	    -state normal
#	.menuBar.file entryconfig "Open Mosaic Segment..." -state normal
#	.menuBar.file entryconfig "Open Mosaic Segment URL..." -state normal

	$buttons(name).file.open configure -state normal

	set count [$current(frame) get fits count]
	if {$count == 0} {
	    .menuBar.file entryconfig "Save as Fits Image..." -state disabled
	    .menuBar.file entryconfig "Display Fits Header" -state disabled
	    .menuBar.frame entryconfig "Clear Frame" -state disabled

	    $buttons(name).file.save configure -state disabled
	    $buttons(name).file.header configure -state disabled
	    $buttons(name).frame.clear configure -state disabled
	} else {
	    .menuBar.file entryconfig "Save as Fits Image..." -state normal
	    .menuBar.file entryconfig "Display Fits Header" -state normal
	    .menuBar.frame entryconfig "Clear Frame" -state normal

	    $buttons(name).file.save configure -state normal
	    $buttons(name).file.header configure -state normal
	    $buttons(name).frame.clear configure -state normal
	}

	.menuBar.frame entryconfig "Match Frames" -state normal
	.menuBar.frame entryconfig "Lock Crosshairs" -state normal

    } else {
	.menuBar.file entryconfig "Open..." -state disabled
#	.menuBar.file entryconfig "Open Array..." -state disabled
	.menuBar.file entryconfig "Open URL..." -state disabled
#	.menuBar.file entryconfig "Open Mosaic Image File..." -state disabled
#	.menuBar.file entryconfig "Open Mosaic Image Multiple Files..." \
	    -state disabled
#	.menuBar.file entryconfig "Open Mosaic Segment..." -state disabled
#	.menuBar.file entryconfig "Open Mosaic Segment URL..." -state disabled
	$buttons(name).file.open configure -state normal
	...
	.menuBar.frame entryconfig "Match Frames" -state disabled
	.menuBar.frame entryconfig "Lock Crosshairs" -state disabled
    }
}


.menuBar.file del "Open Array..."
.menuBar.file del "Open Mosaic Image File..."
.menuBar.file del "Open Mosaic Image Multiple Files..."
.menuBar.file del "Open Mosaic Segment..."
.menuBar.file del "Open Mosaic Segment URL..."

# Subvert Bill's API for minimum disruption
#
set ds9(lastFitsName)		ImageFileClass
set ds9(lastFitsSave)		ImageFileClass
set ds9(lastArrayName)		ArrayFileClass
set ds9(lastColormapName)	CMapFileClass
set ds9(lastMarkerName)		MarkerFileClass
set ds9(lastTextName)		TextFileClass
set ds9(lastTclName)		TclFileClass
set ds9(lastGridName)		GridFileClass
set ds9(lastPointName)		PointFileClass
set ds9(lastContourName)	ContourFileClass
set ds9(lastLevelName)		ConLevFileClass

rename OpenFileDialog _OpenFileDialog
proc   OpenFileDialog { top classname } {
	upvar #0 $classname class

	_OpenFileDialog $top $class
}

rename SaveFileDialog _SaveFileDialog
proc   SaveFileDialog { top classname } {
	upvar #0 $classname class

	_SaveFileDialog $top $class
}

proc OpenFile {} {
    global ds9 FileLoadType FileDialogType
 
    set FileDialogType $ds9(dialog)
    set filename [OpenFileDialog $ds9(top) ds9(lastFitsName)]

    set n [llength $filename]
    ...
    foreach file $filename {
        switch $FileLoadType {
            image                               { LoadFits  $file "" 	}
            array                               { LoadArray $file 	}
            {mosaic image file}                 { LoadMosaicImageFile  $file }
            {mosaic image in multiple files}    { LoadMosaicFiles $file }
            {mosaic image segment}              { LoadFits $file mosaic }
        }
        if { $n > 1 } { CreateFrame }
        incr n -1
    }
...
set imloadtypes {
	...
	{array}					\
	{mosaic image file}			\
	{mosaic image in multiple files}	\
	{mosaic image segment}
    }

proc InitFileClass { Class } {
	upvar $Class class
	global NullFileClass

    set class(file)		$NullFileClass(file) 
    set class(directory)	$NullFileClass(directory)
    set class(dirslist)		$NullFileClass(dirslist)
    set class(loadtype)		$NullFileClass(loadtype)
    set class(pattern)		$NullFileClass(pattern)
    set class(types)		$NullFileClass(types)
}


InitFileClass PointFileClass
set PointFileClass(types) {
    { "Starbase Tables"			*.tab }
    { "Starbase Tables"			*.db }
    { "All files" 			* }
}

InitFileClass GridFileClass
set GridFileClass(types) {
    { "Contour Level File"		*.grd }
    { "All files" 			* }
}

InitFileClass ConLevFileClass
set ConLevFileClass(types) {
    { "Contour Level File"		*.lev }
    { "All files" 			* }
}

InitFileClass ContourFileClass
set ContourFileClass(types) {
    { "Contour File"			*.con }
    { "All files" 			* }
}

InitFileClass MarkerFileClass
set MarkerFileClass(types) {
    { "Marker Files"   		*.mrk 	}
    { "Region Files"   		*.reg	}
    { "All files"               * 	}
}

InitFileClass ImageFileClass
set ImageFileClass(loadtype)  	$imloadtypes
set ImageFileClass(pattern)	 1
set ImageFileClass(types) {
    { "FITS files"              *.fits }
    { "Any FITS files"          *.fits }
    { "Any FITS files"          *.fitz }
    { "Any FITS files"          *.fits.gz }
    { "Any FITS files"          *.fits.Z }
...
InitFileClass ArrayFileClass
set ArrayFileClass(types) {
    { "All files"               * }
}

InitFileClass CMapFileClass
set CMapFileClass(types) {
    { "Color Map Files"		*.map	}
    { "All files"                       * }
}

InitFileClass TclFileClass
set TclFileClass(types) {
    { "tcl files"                       *.tcl }
    { "All files"                       * }
}
