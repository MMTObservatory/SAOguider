...
#
# RCS: @(#) $Id: tkfbox.tcl,v 1.3 2003/10/15 16:41:43 john Exp $
#
...
proc ::tk::IconList_Index {w i} {
	...
	"^end$" {
	    return [expr [llength $data(list)] -1]
	}
	"@-?[0-9]+,-?[0-9]+" {
...
proc ::tk::IconList_Selection {w op args} {
	...
	"set" {
            if {![info exists data(list)]} {set data(list) {}}
	    if { ![llength $data(list)] } { set data(selection) {}; return }
...
proc ::tk::IconList_Create {w} {
    ...
    grid $data(canvas) -sticky news
    grid $data(sbar)   -sticky news
    ...
    bind $data(canvas) <ButtonRelease-1> "tk::CancelRepeat; tk::IconList_PauseBtnCancel $w"
...
proc ::tk::IconList_Btn1 {w x y} {
    ...
    set data(pick) $i
    set data(pausebtn) [after 1250 "::tk::IconList_PauseBtn $w"]
}

proc ::tk::IconList_PauseBtnCancel { w } {
    upvar #0 ::tk::$w data

    catch { after cancel $data(pausebtn) }
    if { [string compare $data(-infoproc) {}] } { $data(-infoproc) $w cancel }
}

proc ::tk::IconList_PauseBtn { w } {
    upvar #0 ::tk::$w data
    if { [string compare $data(-infoproc) {}] } { $data(-infoproc) $w info }
}
...
proc ::tk::IconList_CtrlBtn1 {w x y} {
	...
	if { [IconList_Selection $w includes $i] } {
	    ...
	    IconList_Selection $w set $i
	    set data(pick) $i
	    set data(pausebtn) [after 2000 "::tk::IconList_PauseBtn $w"]
	}
...
proc ::tk::dialog::file:: {type args} {
    ...
    ::tk::IconList_Goto $data(icons) $data(-initialfile)
...
proc ::tk::dialog::file::Create {w class} {
    ...
    #
    if { [string compare $data(-colormap) {}] } {
	toplevel $w -class $class -colormap $data(-colormap)
    } else {
...
static char updir_bits[] = {
   ...
   0xf0, 0xff, 0xff, 0x01};}]
    ...
    set data(icons) [::tk::IconList $w.icons \
	    ...
	    -infoproc	$data(-infoproc) \
	    -multiple	$data(-multiple)]
    bind $data(icons) <<ListboxSelect>> \
	    [list ::tk::dialog::file::ListBrowse $w]

   if $data(-blueplate) {
    set data(dires) [::tk::IconList $w.dires \
	    -command	$iconListCommand]
	    
    bind $data(dires) <<ListboxSelect>> \
	    [list ::tk::dialog::file::ListBrowse $w]
   }
   ...
   if $data(-blueplate) {
    ...
    label $f2a.show -text "  Show Dot:"
    set data(fDots)     [checkbutton $f2a.fdots     -text "Files" \
        -pady 3 -variable ::tk::dialog::file::${dataName}(dotf)	  \
	-command "::tk::dialog::file::UpdateWhenIdle $w"]
    set data(dDots)     [checkbutton $f2a.ddots     -text "Dirs"  \
        -pady 3 -variable ::tk::dialog::file::${dataName}(dotd)	  \
	-command "::tk::dialog::file::UpdateWhenIdle $w"]
    set data(dSAll)     [button $f2a.dsall     -text "Select All" -under 0 -width 6 \
        -pady 3 								\
	-command "::tk::IconList_Selection $data(icons) set 0 end"]
    ...
    set ::tk::$w.dires(font) [$data(ent) cget -font]
   ...
   if $data(-blueplate) {
    ...
    pack $data(dSAll) -side left  -padx 4 -anchor e
   ...
   if { $data(-blueplate) } {
    pack $data(dires) -expand yes -fill both -padx 4 -pady 1
   }
...
rSASvJTGhnhcV3EJlo3kh53ltF5nAhQAOw==}]
   ...
   if $data(-blueplate) {
    ::tk::IconList_DeleteAll $data(dires)
   }
    ...
    if { $data(-blueplate) } {
	set box dires
        if { $data(dotd) } { set dotd .* }
        if { $data(dotf) } { set dotf .* }
    } else {
	set box icons
    }
	...
	if { [string equal $f .] || [string equal $f ..] } { continue }

	if {[file isdir ./$f]} {
	    if { [regexp $dotd $f] } { lappend direList $f }
	} else {
	    if { [regexp $dotf $f] } { lappend fileList $f }
	}
    }

    ::tk::IconList_Add $data($box) $folder $direList
    ...
    if $data(-blueplate) {
	::tk::IconList_Arrange $data(dires)
    }
...
proc ::tk::dialog::file::SetPath {w name1 name2 op} {
	...
	upvar ::tk::$data(icons) idata

        if { [string compare $idata(-infoproc) {}] } {
	    $data(-infoproc) $data(icons) cancel
	}
    }
}
...
#	      = ERROR	: Invalid entry
#             = NOMATCH : Can't glob in multiple mode == File pattern
#
...
proc ::tk::dialog::file::ActivateEnt {w} {
    ...
    if { !$data(-multiple) || [llength $data(selectFile)] == 0 } {
