...
proc Notebook:create {w args} {
  global Notebook
  set Notebook($w,panewidth)    -1
  set Notebook($w,paneheight)   -1
  set Notebook($w,width)  	1
  set Notebook($w,height) 	1
...
proc Notebook:pageconf { w hi wi } {
  global Notebook

  if { $Notebook($w,panewidth)  < $wi } {
    set Notebook($w,panewidth)    [expr $wi+4]
  }
  if { $Notebook($w,paneheight) < $hi } {
    set Notebook($w,paneheight)   [expr $hi+4]
  }

  if { $Notebook($w,width)  != $Notebook($w,panewidth)
    || $Notebook($w,height) != $Notebook($w,paneheight) } {
      after idle "Notebook:config $w  -width  $Notebook($w,panewidth)	\
		      		      -height $Notebook($w,paneheight)"
  }
}
...
proc Notebook:page { w } {
	return [lindex $::Notebook($w,pages) $::Notebook($w,top)]
}
...
proc Notebook:config {w args} {
  ...
  foreach p $Notebook($w,pages) {
    ...
    if {![winfo exists $w.f$cnt]} {
      lappend Notebook($w,frames) [frame $w.f$cnt -bd 0]
      bind $w.f$cnt <Configure> "Notebook:pageconf $w %h %w"
    }

    place $w.f$cnt -x $Notebook($w,x2) -y $Notebook($w,y6)
    $w.f$cnt config -bg $Notebook($w,bg) -width  $Notebook($w,width)	\
					 -height $Notebook($w,height)
    incr cnt
  }
  $w create line \
...
proc Notebook:raise.page {w n} {
  ...
  if {[info exists Notebook($w,change)]} {
    set change $Notebook($w,change)
    if {"$change"!="" && [eval uplevel #0 $change [lindex $Notebook($w,pages) $n]]!=0} {
      return
    }
  }
  ...
  foreach frame $Notebook($w,frames) { place forget $frame }
  place $w.f$n -x $Notebook($w,x2) -y $Notebook($w,y6)
