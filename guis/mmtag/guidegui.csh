...
if ($SAOGUIDER_ROOT == /mmt/saoguider) then
    setenv CCDHOST `/mmt/bin/mmtservdata specguider-ccdhost | tr " " ":"`
else
    setenv CCDHOST 127.0.0.1:2402
endif
setenv inst az
grep '^[a-z]' $SAOGUIDER_ROOT/guis/mmtag/guidserv.unique.rc | cut -d'#' -f1 | cat -n | nc `echo $GUIDSERV | tr ":" " "` > /dev/null
...
setenv MSGINITDIR $SAOGUIDER_ROOT/guis/mmtag
